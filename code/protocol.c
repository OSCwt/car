...
static uint8_t protocol_frame_parse(uint8_t *data, uint16_t *data_len)
{
    ...
    need_to_parse_len = (int16_t)recvbuf_get_len_to_parse(parser.frame_len, PROT_FRAME_LEN_RECV, parser.r_oft, parser.w_oft);    // 得到为解析的数据长度
    ...
    if (0 == parser.found_frame_head)
    {
        ...
        else 
        {
            ...
            for(int i=0;i<need_to_parse_len;i++)
            {
                ips200pro_label_printf(label_ids[i+3],"%02X",parser.recv_ptr[parser.r_oft+i]);
            }
  ...
  while(1)
  {
    ...
    switch (cmd_type)
    {
      ...
      case SET_P_I_D_CMD:
      {
        ...
        PID_debug_get_data( p_temp, i_temp, d_temp);    // 设置 P I D
      }
      break;
      ...
      case SET_TARGET_CMD:
      {
        ...
        PID_get_target((float)actual_temp/100);
      ...
      case STOP_CMD:
      {
          piddebug.MotorEnable=false;              // 停止电机
      }
      break;
...
/**
  ...
  * @retval 无
  */
void uart_transmit(uart_index_enum uart_n, const uint8 *buff, uint8 len)
{
...
void set_computer_value(uint8_t cmd, uint8_t ch, void *data, uint8_t num)
{
  ...
  uart_write_buffer(UART_INDEX,(uint8_t *)&set_packet.head,sizeof(set_packet.head));
  uart_write_buffer(UART_INDEX,(uint8_t *)&set_packet.ch,sizeof(set_packet.ch));
  uart_write_buffer(UART_INDEX,(uint8_t *)&set_packet.len,sizeof(set_packet.len));
  uart_write_buffer(UART_INDEX,(uint8_t *)&set_packet.cmd,sizeof(set_packet.cmd));
...
//    IfxAsclin_Asc_isrReceive(&uart0_handle);
    ...
    uart_query_byte(DEBUG_UART_INDEX, &Uart1Res);
//    ips200pro_label_printf(label_ids[count_1+3],"%02X",Uart1Res);
//    count_1++;
    protocol_data_recv(&Uart1Res, 1);
